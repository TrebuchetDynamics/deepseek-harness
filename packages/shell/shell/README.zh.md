...
后台进程属于 subprocess 服务而非执行器：它能在仅重载执行器后存活，并在组合拆解时被终止并 join。实现必须遵守 seam 的语义——`run` 只在基础设施失败时 reject；`start` 立即返回且不设超时，其 `done` 绝不 reject（spawn 失败以 `killed` 结算、设置 `infrastructureFailed`，并公开一个带合成 `[stderr]` 标记的 `spawn failed: …` 读取增量）；`readOutput` 是消费式的，有损读取会报告 spill 文件。
