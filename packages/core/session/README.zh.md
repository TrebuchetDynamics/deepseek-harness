...
如果恢复发现 assistant 工具请求没有持久 `tool/call`，其合成 `TOOL_NOT_STARTED` 结果内容为 `The tool call was interrupted before the Harness recorded it as started. Retry it if it is still needed.`。如果持久 `tool/call` 没有结果，其 `TOOL_OUTCOME_UNKNOWN` 结果内容为 `The tool call was interrupted after starting; its outcome is unknown. Continue without asking solely because of this interruption. Retry read-only or idempotent calls. Before retrying a call that may have side effects, verify whether it completed; ask the user only when safe verification is impossible or a human-owned choice is required.`。
