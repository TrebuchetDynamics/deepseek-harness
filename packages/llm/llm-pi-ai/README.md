...
- **Provider HTTP status is unavailable** — pi-ai error events do not expose a stable HTTP status across providers. The adapter classifies known flattened messages: explicit HTTP 400/413 or request-size evidence maps to non-retryable `INVALID_REQUEST` even when the same text contains OpenAI's generic retry advice; that generic processing failure alone maps to retryable `SERVER`, while unknown wording remains non-retryable `PI_AI_ERROR`.
