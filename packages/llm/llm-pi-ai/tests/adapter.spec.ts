...
describe('PiAiAdapter provider routing', () => {
  ...
  it('defaults direct-adapter image limits and resolves later attachment and filesystem services', async () => {
    const server = await mockServer([{ status: 401, body: JSON.stringify({ error: { message: 'expected mock failure' } }) }])
    ...
    ctx.llm.registerAdapter(['openai'], new PiAiAdapter({
      ...
      resolveImageAccess: (attachments, imageRef) => resolveImageAttachmentAccess(
        attachments,
