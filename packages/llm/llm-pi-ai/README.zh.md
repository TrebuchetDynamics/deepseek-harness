...
包根入口导出 Cordis 插件约定、`PiAiAdapter` 与 `supportedProtocols()`；profile 解析、catalog 物化、提供方构造、回放转换和流转换保留在包内部。直接使用 `PiAiAdapter` 的集成提供已验证的 `PiAiAdapterProfile`，但可以省略 `maxRequestImageBytes`、`requestImagePixelBudget` 与 `requestImageMaxBytes`；适配器会在转换前解析出与 Cordis profile resolver 相同的图像默认值。
