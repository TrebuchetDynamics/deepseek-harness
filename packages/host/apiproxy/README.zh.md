...
一次被接受的 `session.prompt` 会取代该 Session 中所有等待中的 `ask_user_question`。Host 先把普通提示词接纳进 Agent 收件箱，再取消等待中的问题；被拒绝的提示词不会让问题失去应答能力，而已接受的 steering 或 follow-up 消息也不会永远堵在它必须取代的工具调用之后。
