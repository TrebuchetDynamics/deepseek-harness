...
声明步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。所有生产浏览器都会通过既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确点击「继续」才会记录当前版本。Host 仍是授权权威，因此经过认证的远程所有者会获得持久确认，而被拒绝的远程请求无法修改它。显式使用内存模式的消费者仅在当前进程保留确认。
