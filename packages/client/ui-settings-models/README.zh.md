...
声明步骤在 `src/client/locales.ts` 中持有精确文案，并在 `src/onboarding-copy.ts` 中持有确认版本。每个生产浏览器都通过 settings Remote 比较并写入 `ui-onboarding.welcomeNoticeVersion`，且只有显式点击「继续」才会记录当前版本。Host 仍是授权权威，因此经过认证的远程所有者会获得持久确认，而被拒绝的远程请求无法修改它；显式使用内存模式的消费者仅在当前进程保留确认。DeepSeek 步骤在共享引导模态框内以仅凭据模式渲染既有 `ProviderEditor`；`credentials.set` 仍是唯一的机密写入，且不改变任何提供方设置。
