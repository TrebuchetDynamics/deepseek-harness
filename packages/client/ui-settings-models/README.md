...
The notice step owns its exact copy in `src/client/locales.ts` and its acknowledgement version in `src/onboarding-copy.ts`. Every production browser compares and writes `ui-onboarding.welcomeNoticeVersion` through the settings Remote, and only an explicit Continue records the current version. The Host remains the authorization authority, so an authenticated remote owner receives durable acknowledgement while a rejected remote request cannot modify it; explicit in-memory consumers keep acknowledgement process-local. The DeepSeek step renders the existing `ProviderEditor` in credential-only mode inside the shared onboarding modal; `credentials.set` stays the only secret write, and no provider settings are changed.
