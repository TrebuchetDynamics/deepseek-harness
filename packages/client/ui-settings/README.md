...
- **Remote settings require Host authorization** — browsers attempt settings RPCs regardless of their URL; the Host remains authoritative and rejects unapproved remote requests, while an authenticated reverse proxy may present an approved authority for its owner.
