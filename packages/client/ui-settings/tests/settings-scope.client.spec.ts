...
describe('SettingsScopeController', () => {
  ...
  it('keeps an explicit memory-mode consumer free of Host calls', async () => {
    const describeCall = vi.fn()
    const mutate = vi.fn()
    const ctx = ctxWith({ describe: describeCall, mutate })
...
describe('SettingsScopeBinder.bind', () => {
  ...
  it('binds a remote browser through the Host-authorized settings transport', async () => {
    const describeCall = vi.fn().mockResolvedValue(described({ preference: 'dark' }, 1))
    const ctx = new Context()
  ...
  it('binds an explicit memory-mode consumer without starting a settings read', async () => {
    const describeCall = vi.fn()
