...
export class UserQuestionService extends Service {
  private readonly pendingByAgent = new Map<Agent, Set<AbortController>>()

  constructor(ctx: Context) {
    super(ctx, 'userQuestions')
    ctx.on('agent/inbox/inserted', ({ agent, message }) => {
      if (message.source.kind !== 'user') return
      for (const controller of this.pendingByAgent.get(agent) ?? []) {
        controller.abort(new UserQuestionError(
          'the user sent a new prompt instead of answering ask_user_question',
          'ASK_CANCELLED',
        ))
      }
    })
  ...
  async ask(request: AskUserQuestionRequest): Promise<AskUserQuestionAnswer> {
    ...
    const superseded = agent === undefined ? undefined : new AbortController()
    if (agent !== undefined && superseded !== undefined) {
      const pending = this.pendingByAgent.get(agent) ?? new Set<AbortController>()
      pending.add(superseded)
      this.pendingByAgent.set(agent, pending)
    }
    let dispatched: AskUserQuestionRequestEvent = request
    if (superseded !== undefined && agent !== undefined) {
      dispatched = {
        ...request,
        agent,
        signal: request.signal === undefined
          ? superseded.signal
          : AbortSignal.any([request.signal, superseded.signal]),
      }
    }
    try {
      ...
      if (agent !== undefined && superseded !== undefined) {
        ...
        if (pending?.size === 0) this.pendingByAgent.delete(agent)
      }
    }
  }
}
