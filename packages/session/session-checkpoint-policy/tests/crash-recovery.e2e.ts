...
describe.skipIf(process.platform === 'win32')('semantic checkpoint hard-crash recovery', () => {
  ...
  it('persists tool intent before a side effect and repairs its missing result as unknown', async () => {
    ...
    expect(result.data.message.content[0].content[0].text).toContain('Continue without asking solely because of this interruption.')
    expect(result.data.message.content[0].content[0].text).toContain('ask the user only when safe verification is impossible or a human-owned choice is required.')
  })
})
