...
The host must use systemd and have a connected host Tailscale node. The service user's non-interactive login shell must provide Caddy 2, Node.js `^22.19.0 || >=24.0.0`, pnpm `11.7.0`, Git, curl, Tailscale, `setsid`, and `ss`; the host also needs `sudo`, `systemctl`, `systemd-analyze`, `runuser`, `getent`, `install`, `flock`, and `readlink`. On Ubuntu and Fedora, installation uses the configured APT or DNF repositories to install a missing Caddy package before checking versions. Other dependencies, including Node.js and pnpm, remain user-managed login-shell tools.
...
Install a supported Node.js release, then pin pnpm separately:
...
Harness and Caddy bind only to `127.0.0.1`. Host Tailscale Serve terminates HTTPS and supplies the authenticated `Tailscale-User-Login`; [`Caddyfile`](Caddyfile) restricts settings, credentials, model discovery, preset management, native host operations, Remote SSH, and task-board control routes to `TAILSCALE_OWNER`. Tailnet ACLs still decide who can reach the GUI, and every admitted GUI user can run ordinary agent tools with the service user's host permissions.
...
- Ubuntu and Fedora installation may install the missing Caddy package after masking its package service for the transaction. It does not configure third-party repositories or install or upgrade other dependencies; Arch reports the manual command instead.
