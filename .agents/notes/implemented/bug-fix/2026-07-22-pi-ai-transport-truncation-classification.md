# Agent Note: Classify retryable pi-ai failures from flattened message text
...
A TUI run whose model connection dropped mid-stream surfaced the single notice `terminated`, and a truncated Anthropic response surfaced `Anthropic stream ended before message_stop`. Both are transport truncations — the connection died before the provider's terminal SSE event — yet `classifyPiAiError` in `dsh-llm-pi-ai` mapped neither, falling through to the catch-all `PI_AI_ERROR`. OpenAI Codex likewise returned `An error occurred while processing your request. You can retry your request` without its HTTP status, so the explicitly transient provider failure reached the same catch-all. Because `PI_AI_ERROR` is not in `llm-retry`'s `DEFAULT_RETRYABLE_CODES` (`RATE_LIMIT`, `SERVER`, `TIMEOUT`, `TRANSPORT`), these recoverable failures were treated as permanent and never retried.
...
- OpenAI's generic `An error occurred while processing your request` response maps to `SERVER`; its own text instructs the caller to retry even though pi-ai omitted the status. Explicit HTTP 400/413 or request-size evidence in the same flattened message takes precedence and maps to non-retryable `INVALID_REQUEST`; status matching requires an HTTP/error form rather than a bare number inside a request id.
...
- A mid-stream transport drop and a pre-terminal stream truncation carry `TRANSPORT`, while OpenAI's generic processing failure carries `SERVER`; a composed `llm-retry` policy retries all three by default. Explicit invalid-request evidence prevents that retry for a rejected request that cannot succeed unchanged.
