...
- OpenAI's generic `An error occurred while processing your request` response maps to `SERVER`; its own text instructs the caller to retry even though pi-ai omitted the status.
...
- A mid-stream transport drop and a pre-terminal stream truncation now carry `TRANSPORT`, while OpenAI's generic processing failure carries `SERVER`; a composed `llm-retry` policy retries all three by default instead of failing the turn.
