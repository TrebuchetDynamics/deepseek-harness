...
`run-docker.sh` 区分硬性先决条件与可选工具链。硬性先决条件——`PATH` 上的 `docker`、`node`、`curl`、`tailscale`，存在的宿主 home，以及存在的仓库——仍然以 `error:` 消息中止。其余一切以 `warning:` 消息降级，并输出一行 `host toolchains:` 摘要：
...
- Docker 通过 `command -v docker` 解析，而不是硬编码路径。
...
启动器现在可以在只有任意工具链子集的宿主上启动：只有 Flutter 与 Java 的机器（报告中的情形）带一条警告启动且没有 Android 挂载，没有任何工具链或 `~/git` 的裸服务器也能提供 GUI。在原来那台全配置机器上行为不变——相同的环境值、相同的挂载（包括 `/usr/lib/android-sdk` 与只读 JDK）——由 stub 宿主套件（57 条断言：覆盖 home 内 SDK、仅发行版路径、环境变量 `ANDROID_HOME`、有效与无效显式覆盖、裸宿主等八种宿主场景，加一次真实工具链冒烟运行）以及用真实 Compose v5.5.0 二进制执行的 `docker compose config` 验证，确认了嵌套默认值插值与基础+override 的 volume 并集且保留 `:ro`。
...
验证装置位于仓库之外（stub 的 `docker`/`tailscale`/`curl` 二进制加上 sed 改写过的探测路径），因为仓库的 vitest 套件覆盖的是包而不是宿主 shell 脚本；在启动器中重新引入硬编码宿主路径不会让任何仓库内 gate 失败。
