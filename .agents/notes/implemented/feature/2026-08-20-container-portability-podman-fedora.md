...
Runtime detection replaces the `docker` hard requirement: `docker` on `PATH` wins, otherwise `podman compose` (which delegates to a `docker-compose` or `podman-compose` provider), otherwise the `podman-compose` wrapper, otherwise the launcher dies with an instruction. The detected command array is used for every Compose invocation. `node`, `curl`, and `tailscale` stay hard requirements (the launcher parses `tailscale status` JSON with `node`).
...
Trade-offs: `podman compose` requires a provider binary and (for the `docker-compose` provider) a running `podman.socket` — the README's Fedora notes give the three setup commands; `restart: unless-stopped` under rootless podman only survives logout with `loginctl enable-linger`, also documented. The `podman-docker` alias package makes `docker` resolve to a podman shim; the launcher then treats it as Docker and skips rootless detection, so the README tells alias users to invoke `podman` directly. `label=disable` removes SELinux confinement for these two services — acceptable because the services are machine-local, loopback-bound, and already hold read-write access to the home by design; the confinement they lose is not one the composition relied on. Standalone `docker run` (no Compose) no longer persists Tailscale state via an anonymous volume; Compose runs keep it via `tsstate`.

Verification: the out-of-repo stub-host suite grew from 57 to 98 assertions. New scenarios: rootless podman (compose delegation logged, `keep-id` plus volumes in the generated override, `DSH_UID`/`DSH_GID` exported), rootful podman (no `keep-id`), podman without a compose provider (dies with the install instruction), the `podman-compose` wrapper path (works without a `podman` binary), a host without `stat` (falls back to the invoking user with a warning), and no container runtime at all (dies with an instruction). Entrypoint and Dockerfile contract greps pin the uid-agnostic `setpriv` drop, the chown-before-`tailscaled` ordering, and the label/cache/VOLUME changes; the real Compose v5.5.0 binary renders `init`, `security_opt`, `DSH_UID`/`DSH_GID` interpolation, and the `userns_mode` merge (absent from the base render, present with the rootless override) — this extends the 2026-08-19 launcher-degradation note, which previously listed `docker` among the hard prerequisites.
